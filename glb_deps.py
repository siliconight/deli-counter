...
WHY THIS EXISTS. `portable_building.build_package` bundles a building's art by
resolving each `res://art/zoo/<name>.glb` out of the scene it just wrote and
calling `shutil.copy2` on that one path. That was exactly right while a Zoo
module carried its images inside its binary chunk, and it stayed exactly the
same line when Zoo 1.2.0 stopped: the modules now name their textures with a
relative glTF `images[].uri`, and a package built by that copy contains every
GLB and none of the pixels.
...
which 1,136 came through this repo's composer. The walker's report was "around
90% graybox".

NO LIBRARY, AND NO `pygltflib`. This reads the JSON chunk and nothing else --
it never rewrites the file, so parsing the container is the whole job, and a
dependency that loads and re-saves a GLB to answer "what does it name" would
be free to change bytes this module has no business changing.
